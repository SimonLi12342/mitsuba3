# Good intro to conjugate gradient:
# https://www2.math.ethz.ch/education/bachelor/lectures/hs2014/math/nummath_cse/Intro_CG.pdf
...
def JT_J_x(x_eval, x_input, sc, sensor_index = None):
    """
    ...
    image, y = sc.eval_func(x, sensor_idx = sensor_index, forward_param=alpha)
    # print("y grad enabled: ", dr.grad_enabled(y))
    # dr.enable_grad(y)
    dr.forward_to(y)
    ...
    return JT_y(x_eval, J_x, sc, sensor_index = sensor_index)


def JT_y(x_eval, y_input, sc, sensor_index = None):
    """
    ...
    image, y = sc.eval_func(x, sensor_idx = sensor_index)
    #dr.enable_grad(y)
    # print("y: ", y)
...
def conjugate_gradient(x_eval, residual, sc, sensor_idx = None):
    """
    ...
    # print("x_eval grad enabled: ", dr.grad_enabled(x_eval))
    
    xs = dr.width(x_eval)
    dir_x = dr.zeros(mi.Float, xs)          # [xs, ]: the descent direction to be computed
    r_curr = -JT_y(x_eval, residual, sc, sensor_index = sensor_idx)    # [xs, ]: the residual of conjugate gradient
    #r_descent_init = -r_curr                # gradient
    ...
    for i in range(10):
        ...
        Hd = JT_J_x(x_eval, d_curr, sc, sensor_index = sensor_idx) # TODO: question: Hessian-vector product use finite difference?
        ...
        r_next = r_curr - alpha * Hd  # TODO plus?
        ...
        if True:
            ...
            if dr.slice(dr.norm(r_next)) < 1e-5:
...
class SceneManager():
    ...
    def update(self, values):
        # print("real value: ", values)
        
        # values_flat = {}
        # start_i = 0
        # for key in self.keys:
        #     values_flat[key] = values[start_i: start_i + self.len_values[key]]
        #     start_i += self.len_values[key]
        
        # for key in self.keys:
        #     value = values_flat[key]
        #     dr.enable_grad(value)
        #     print("value grad enabled: ", dr.grad_enabled(value))
        #     print("wrong value: ", value)
        #     self.params[key] = dr.unravel(type(self.params[key]), value)
        
        # start_i = 0
        # print(dr.grad_enabled(values))
        # print(dr.grad_enabled(values[start_i: len(values)]))
        start_i = 0
        for key in self.keys:
            # # print("indexed value grad enabled: ", dr.grad_enabled(values[0 : self.len_values[key]]))
            # # print("value grad enabled: ", dr.grad_enabled(values))
            ...
            #print(type(self.params[key].array))
            # print("param data grad enabled: ", dr.grad_enabled(param_data))
            param_reshape = mi.TensorXf(param_data, shape = self.params[key].shape)
            # print("param reshaped grad enabled: ", dr.grad_enabled(param_reshape))
            
            self.params[key] = param_reshape
            #self.params[key] = dr.unravel(type(self.params[key]), param_data)
            start_i += self.len_values[key]
            self.params.update()
    ...
    def step(self, descent_dir):
        # TODO change to multiple values
        curr_val = self.values()
        new_val = curr_val + descent_dir 
        # TODO line search with armijo/ Wolfe/ Goldstein condition 
        # to avoid local minima useful?
